<li id="event-{{ $event->id }}" class="event-card" style="display: flow-root;">
	...
	{!! link_to_route('events.show', $event->name, [$event->id], ['class' => 'item-title', 'alt' => $event->name,
	...
	<P>
		...
		<span class="label label-tag"><a href="/events/tag/{{ urlencode($tag->name) }}" class="label-link">{{ $tag->name
				}}</a>
