<li id="event-{{ $event->id }}" class="event-card {{ $event->pastOrFuture }} {{ $event->visibility->name === 'Cancelled' ?  "event-cancelled" : ""}} flow-root">
	...
	<div class="event-list-thumbnail">
		<a href="{{ Storage::disk('external')->url($primary->getStoragePath()) }}" 
			...
			<img src="{{ Storage::disk('external')->url($primary->getStoragePath()) }}" alt="{{ $event->name }}" class="thumbnail-image"  width="100" height="100">
		</a>
	</div>
	@else
	<div class="event-list-thumbnail">
	...
	<span>
		...
		@if ($response = $event->getEventResponse($signedIn ? $user : null))
		...
		@else
		<a href="{!! route('events.attend', ['id' => $event->id]) !!}" data-target="#event-{{ $event->id }}" class="ajax-action" title="{{ $signedIn ? 'Click star to mark attending' : 'Sign in to mark attending' }}">
