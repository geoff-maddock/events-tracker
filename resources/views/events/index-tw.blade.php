...
<div class="mb-6 flex flex-wrap items-start gap-2">
	...
	@if($hasFilter)
	<div id="active-filters-badges" class="flex flex-wrap items-center gap-2">
...
<div id="filter-panel" class="hidden bg-card border border-border rounded-lg p-4 mb-6 overflow-hidden">
	<form method="POST" action="{{ route($filterRoute ?? 'events.filter') }}" accept-charset="UTF-8" name="filters">
	@csrf
	...
	<div class="flex gap-2 mt-4">
		...
		<input type="hidden" name="redirect" value="{{ $redirect ?? 'events.index' }}">
		<input type="hidden" name="key" value="{{ $key ?? 'internal_event_index' }}">
