<div class="group bg-dark-surface rounded-lg overflow-hidden shadow-lg border border-dark-border hover:border-primary/50 hover:shadow-primary/10 transition-all duration-200 h-full flex flex-col">
    <!-- Image Section -->
    <div class="relative aspect-[4/3] overflow-hidden bg-dark-bg">
    ...
    <div class="p-3 flex-1 flex flex-col">
        <!-- Event Title -->
        <h3 class="line-clamp-2 text-sm font-semibold leading-tight text-white mb-2 group-hover:text-primary transition-colors">
        ...
        @if ($event->tags->count() > 0)
            <div class="mt-auto pt-2 flex flex-wrap gap-1">
                @foreach ($event->tags->take(3) as $tag)
                    <span class="inline-flex items-center px-1.5 py-0.5 rounded text-[10px] font-medium bg-dark-card text-gray-300 border border-dark-border">
                        {{ $tag->name }}
                    </span>
                @endforeach
            </div>
