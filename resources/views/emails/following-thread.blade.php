...
<div class='event-date'>
    ...
    {!! $thread->start_at->format('h:i A') !!} {!! $event->end_time ? 'until '.$event->end_time->format('h:i A') : '' !!}
</div>
...
<P>
    @unless ($thread->series->isEmpty())
        ...
        @foreach ($thread->series as $s)
            <span class="label label-tag"><a href="{{ url('threads/relatedto/'.$series->slug) }}">{{ $series->name }}</a></span>
        @endforeach
    @endunless
</P>
