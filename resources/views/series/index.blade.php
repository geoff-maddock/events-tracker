        ...
        <div id="filters-content" class="col-lg-9">
			...
			<!-- BEGIN: FILTERS -->
				...
				<div class="form-group col-sm-2">
                    ...
                    {!! Form::select('filter_occurrence_type', $types, (isset($filters['filter_occurrence_type']) ? $filters['filter_occurrence_type'] : NULL), ['class' =>'form-control']) !!}
                </div>

                <div class="form-group col-sm-2">
                    {!! Form::label('filter_occurrence_week','Week') !!}
                    <?php $weeks = [''=>''] + App\OccurrenceWeek::orderBy('id','ASC')->pluck('name','name')->all();?>
                    {!! Form::select('filter_occurrence_week', $weeks, (isset($filters['filter_occurrence_week']) ? $filters['filter_occurrence_week'] : NULL), ['class' =>'form-control']) !!}
                </div>

                <div class="form-group col-sm-2">
                    {!! Form::label('filter_occurrence_day','Day') !!}
                    <?php $days = [''=>''] + App\OccurrenceDay::orderBy('id','ASC')->pluck('name','name')->all();?>
                    {!! Form::select('filter_occurrence_day', $days, (isset($filters['filter_occurrence_day']) ? $filters['filter_occurrence_day'] : NULL), ['class' =>'form-control']) !!}
                </div>
