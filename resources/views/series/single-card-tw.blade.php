<li id="series-{{ $series->id }}" class="group relative flex flex-col gap-3 p-3 bg-card rounded-lg border border-border hover:border-primary transition-colors shadow-sm min-w-0 h-full">
    <!-- Thumbnail -->
    ...
    <div class="flex-grow min-w-0 flex flex-col">
        ...
        <div class="flex flex-wrap gap-1 mb-2">
            @foreach ($series->entities->take(3) as $entity)
                <x-entity-badge :entity="$entity" context="series" size="sm" />
            @endforeach
            @if ($series->entities->count() > 3)
            <span class="text-xs text-muted-foreground self-center">+{{ $series->entities->count() - 3 }}</span>
            @endif
        </div>
        @endunless
        ...
        <div class="flex flex-wrap gap-1 mt-auto">
            @foreach ($series->tags->take(3) as $tag)
                <x-tag-badge :tag="$tag" context="series" size="sm" />
            @endforeach
