<li id="series-{{ $series->id }}" class="series-card">
    ...
    <a href="{!! route('series.follow', ['id' => $series->id]) !!}" data-target="#series-{{ $series->id }}"
        class="ajax-action card-actions mx-1" title="{{ $signedIn ? 'Click to follow' : 'Sign in to follow' }}" alt="Follow {{ $series->name}}" aria-label="Follow {{ $series->name }}">
    ...
    {!! link_to_route('series.show', $series->name, [$series->slug], ['class' => 'item-title', 'alt' => $series->name, 'aria-label' => $series->name]) !!}
    </div>
    ...
    <br>Next Event is {!! link_to_route('events.show', $event->name, [$event->slug], ['class' =>'butt']) !!}
    @endif
