@if (isset($tag) && (strtolower($tag) === strtolower($t->name)))
	...
	<li class='list selected' id="tag-{{ $tag->id }}"><a href="/tags/{{ $t->name }}" title="Click to show all related events and entities.">{{ $t->name }}</a>
		...
		@else
			<a href="{!! route('tags.follow', ['id' => $t->id]) !!}" data-target="#tag-{{ $t->id }}" class="ajax-action" title="{{ $signedIn ? 'Click to follow' : 'Sign in to follow' }}"><i class="bi bi-plus-circle-fill text-info"></i></a>
		@endif
	</li>
@else
