				...
				@if (isset($permissions) && count($permissions) > 0)
					@foreach ($permissions as $permission)
						<tr class="hover:bg-muted/50 transition-colors">
							<td class="px-4 py-3 text-sm text-muted-foreground">{{ $permission->id }}</td>
							<td class="px-4 py-3">
								<a href="{{ route('permissions.show', $permission->id) }}" class="text-primary hover:underline font-medium">
									{{ $permission->name }}
								</a>
							</td>
							<td class="px-4 py-3 text-sm text-foreground">{{ $permission->label }}</td>
							<td class="px-4 py-3 text-sm text-muted-foreground">{{ $permission->level }}</td>
							<td class="px-4 py-3 text-right">
								<div class="flex items-center justify-end gap-2">
									@can('edit_permission')
										<a href="{{ route('permissions.edit', $permission->id) }}" class="p-2 hover:bg-muted rounded transition-colors" title="Edit">
											<i class="bi bi-pencil text-primary"></i>
										</a>
										<form action="{{ route('permissions.destroy', $permission->id) }}" method="POST" class="inline" data-confirm="Are you sure you want to delete this permission?">
											@csrf
											@method('DELETE')
