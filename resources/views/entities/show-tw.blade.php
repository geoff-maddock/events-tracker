...
<!-- Back Button -->
<div class="mb-6">
	<a href="{{ session('previous_url', URL::previous()) }}" class="inline-flex items-center gap-2 px-3 py-2 text-sm border rounded-lg hover:bg-accent transition-colors">
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        if (typeof EmbedLoader !== 'undefined') {
            EmbedLoader.invalidateCache('entities', '{{ $entity->slug }}');
        }
        @endif
        ...
        if (refreshEmbedsBtn) {
            refreshEmbedsBtn.addEventListener('click', function(e) {
                ...
                if (typeof EmbedLoader !== 'undefined' && slug) {
                    ...
                    EmbedLoader.refresh('entities', slug).then(function(data) {
                        // Find the playlist container and update it
                        var playlistEl = document.querySelector('.playlist-id[data-resource-type="entities"]');
